...
"""
    ...
    style_prompt = forms.CharField(
        widget=forms.TextInput(attrs={'placeholder': '예: 밝고 캐주얼한 느낌'}),
        label='원하는 영상 스타일 (설명)',
        required=False,
    ...
    font_color = forms.ChoiceField(
        choices=[('black', '검정'), ('white', '흰색'), ('red', '빨강'), ('blue', '파랑'), ('green', '초록')],
        label='자막 글자 색상',
        initial='white',
    )
    font_size = forms.ChoiceField(
        choices=[('small', '작게'), ('medium', '보통'), ('large', '크게')],
        label='자막 글자 크기',
        initial='medium',
    )
    comment_text = forms.CharField(
        widget=forms.Textarea(attrs={'rows': 3, 'placeholder': '영상 첫 댓글을 입력하세요'}),
        label='첫 댓글',
        required=False,
        initial='이 영상과 관련된 자세한 설명은 블로그에서 보세요! 👉 https://nicednjsdud.github.io/backend/Backend-cpu_scheduling_blog'
    )
